...
export const PredictionItem: FC<Props> = ({
	...
	const [localCosmeticsMap, setLocalCosmeticsMap] = useState<
		Record<string, any>
	>({});
	...
	const author =
		...
		(typeof p.user_id === "string" &&
			(usersMap[p.user_id] || effectiveUsersById[p.user_id]?.username)) ||
		(typeof p.user_id === "object" &&
			(p.user_id.username ||
				usersMap[String(p.user_id._id)] ||
				effectiveUsersById[String(p.user_id._id)]?.username)) ||
		"inconnu";
	...
	const isMine = Boolean(
		currentId &&
			...
			((typeof p.user_id === "string" && p.user_id === currentId) ||
				(typeof p.user_id === "object" &&
					p.user_id._id &&
					String(p.user_id._id) === currentId)),
	);
		...
		<li key={p._id} className="p-3 border rounded bg-white text-black">
			<div className="flex justify-between items-start">
				<div>
					...
					{(() => {
						const resolvedUser =
							p.user && typeof p.user === "object"
								? p.user
								: p.user_id && typeof p.user_id === "string"
									? effectiveUsersById[String(p.user_id)]
									: p.user_id && typeof p.user_id === "object"
										? p.user_id
										: null;
						const pubAuthor =
						...
						if (userToCheck && userToCheck.currentCosmetic) {
							if (typeof userToCheck.currentCosmetic === "object")
								appliedCosmetic = userToCheck.currentCosmetic;
							else
								appliedCosmetic =
									effectiveCosmeticsMap[
										String(userToCheck.currentCosmetic)
									] || null;
						}
					...
					<p className="text-xs">
						Fin: {new Date(p.dateFin).toLocaleString()}
					</p>
				</div>
