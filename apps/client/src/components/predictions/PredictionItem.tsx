...
export const PredictionItem: FC<Props> = ({
  p,
  usersMap,
  cosmeticsMap,
  usersById,
  ...
  useEffect(() => {
    if (!cosmeticsMap) {
      (async () => {
        try {
          const res = await fetch(`${API_URL}/cosmetic`, {
            headers: token ? { Authorization: `Bearer ${token}` } : {},
          });
          const cosArr = await res.json();
          const cmap: Record<string, any> = {};
          (cosArr || []).forEach((c: any) => {
            if (c && c._id) cmap[String(c._id)] = c;
          });
  ...
  const renderAuthorLine = (maybeUser: any) => {
    const resolvedUser = maybeUser && typeof maybeUser === "object" ? maybeUser : maybeUser && typeof maybeUser === "string" ? effectiveUsersById[String(maybeUser)] : null;
    const pubAuthor = resolvedUser?.username ?? author ?? "inconnu";

    let appliedColor: any = null;
    let appliedBadge: any = null;

    if (resolvedUser && resolvedUser.currentCosmetic) {
      if (Array.isArray(resolvedUser.currentCosmetic)) {
        const colorId = resolvedUser.currentCosmetic[0];
        const badgeId = resolvedUser.currentCosmetic[1];
        if (colorId) appliedColor = effectiveCosmeticsMap[String(colorId)];
        if (badgeId) appliedBadge = effectiveCosmeticsMap[String(badgeId)];
      } else if (typeof resolvedUser.currentCosmetic === "object") {
        if (resolvedUser.currentCosmetic.type === "color") appliedColor = resolvedUser.currentCosmetic;
        else if (resolvedUser.currentCosmetic.type === "badge") appliedBadge = resolvedUser.currentCosmetic;
      } else {
        appliedColor = effectiveCosmeticsMap[String(resolvedUser.currentCosmetic)] || null;
      }
    }

    return (
      <p className="text-xs text-gray-500">
        Par: {appliedColor && appliedColor.type === "color" && appliedColor.hexColor ? (
          <span style={{ color: appliedColor.hexColor }}>{pubAuthor}</span>
        ) : (
          <span>{pubAuthor}</span>
        )}
        {appliedBadge ? <span> {renderWithEmojis(appliedBadge.name)}</span> : null}
      </p>
    );
  };
    ...
    <li key={p._id} className="p-3 border rounded bg-white text-black">
      <div className="flex justify-between items-start">
        <div>
          ...
          {renderAuthorLine(p.user ?? p.user_id)}
      ...
      {isMine ? (
        <div className="mt-2">
          <button className="text-red-600 text-sm" onClick={() => onDelete(p._id)} disabled={deletingId === p._id}>
            {deletingId === p._id ? "Suppression..." : "Supprimer"}
          </button>
        </div>
      ) : null}
