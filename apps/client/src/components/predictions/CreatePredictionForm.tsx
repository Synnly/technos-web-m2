...
export default function CreatePredictionForm({ username, fetchPredictions, onClose, setToast, setError }: Props) {
    ...
    const submitPrediction = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            onClose();
            setTitle(''); setDescription(''); setDateFin(''); setOptions({}); setOptionKey('');
            setLocalError(null);
        } catch (err: any) {
            console.error(err);
    ...
    return (
        <form className="mb-6 space-y-3" onSubmit={submitPrediction}>
            <InputText label="Titre" name="title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Titre de la prédiction" />

            <InputText label="Description" name="description" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Courte description" />

            <InputText label="Date de fin" name="dateFin" type="date" value={dateFin} onChange={(e) => setDateFin(e.target.value)} min={todayStr} />
                    ...
                    <ul className="text-sm space-y-1">
                        {Object.entries(options).map(([k]) => (
                            <li key={k} className="flex items-center justify-between">
                                <span>{k}</span>
                                <button type="button" onClick={() => setOptions((prev) => { const next = { ...prev }; delete next[k]; return next; })} className="text-red-500">Supprimer</button>
                            </li>
                        ))}
                    </ul>
                </div>
