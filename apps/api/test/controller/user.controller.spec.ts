...
describe("UserController", () => {
	...
	describe("createUser", () => {
		it("should return a 400 if the user doesn't have a password", async () => {
			...
			await expect(userController.createUser(newUser)).rejects.toThrow(
				BadRequestException,
			);
		});
	});
	...
	describe("updateUserByUsername", () => {
		it("should return a 200 when a user is updated", async () => {
			// Configuration du mock pour retourner l'utilisateur créé
			mockUserService.createOrUpdateByUsername.mockResolvedValue(
				expectedUser1,
			);
			...
			await userController.updateUserByUsername(
				mockReq,
				expectedUser1.username,
				expectedUser1,
			);

			expect(userService.createOrUpdateByUsername).toHaveBeenCalledWith(
				expectedUser1.username,
				expectedUser1,
			);
		});
	...
	describe("deleteUser", () => {
		it("should return a 404 when the user doesn't exist", async () => {
			...
			await expect(userController.deleteUser(username)).rejects.toThrow(
				NotFoundException,
			);
	...
	describe("getDailyReward", () => {
		...
		it("should return a 400 when no username is provided", async () => {
			...
			await expect(
				userController.getDailyReward(username),
			).rejects.toThrow(BadRequestException);
		});
		...
		it("should return a 400 when daily reward already claimed today", async () => {
			...
			mockUserService.claimDailyReward.mockImplementation(() => {
				throw new Error(
					"Récompense quotidienne déjà réclamée aujourd'hui.",
				);
			});
