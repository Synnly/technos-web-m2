...
describe("CosmeticService", () => {
	...
	beforeEach(async () => {
		...
		const moduleRef = await Test.createTestingModule({
			providers: [CosmeticService, { provide: getModelToken(Cosmetic.name), useValue: mockCosmeticModel }],
		}).compile();
	...
	describe("create", () => {
		it("should call model to create cosmetic", async () => {
			const newCosmetic = {
				name: "Blue Color",
				cost: 80,
				type: CosmeticType.COLOR,
	...
	describe("updateById", () => {
		it("should call model to update cosmetic when found", async () => {
			mockCosmeticModel.findByIdAndUpdate.mockReturnValue({
				exec: jest.fn().mockResolvedValue({ ...expectedCosmetic1, name: "Updated Badge" }),
			});

			await expect(
				cosmeticService.updateById("507f1f77bcf86cd799439011", { name: "Updated Badge" } as Cosmetic),
